package com.herbalife.myapplication.camera.adapter;
...
//        }
        if (sticker.id == 0){
            holder.logo.setImageResource(R.drawable.img_xk_none);
            return convertView;
        }
        if (sticker.isLight){//解锁了
            holder.logo.setImageResource(sticker.id);
        }else{
            holder.logo.setImageResource(sticker.id2);
        }
